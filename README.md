...
If using bundler, add this line to Gemfile  
```
gem 'redtrack'
```
...
```smallint``` Redtrack expects [http://www.ruby-doc.org/core-2.1.5/Integer.html](Ruby Integer). Does not check for overflow. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html](redshift numeric types documentation)) <br/>
```integer``` Redtrack expects [http://www.ruby-doc.org/core-2.1.5/Integer.html](Ruby Integer). Does not check for overflow. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html](redshift numeric types documentation)) <br/>
```bigint``` Redtrack expects [http://www.ruby-doc.org/core-2.1.5/Integer.html](Ruby Integer). Does not check for overflow. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html](redshift numeric types documentation))<br/>
```varchar(n)``` Redtrack expects [http://www.ruby-doc.org/core-2.1.4/String.html](Ruby String). Redtrack will accept and truncate any strings that exceeds the provided length. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Character_types.html](redshift character type documentat))<br/>
```char(n)``` Redtrack expects [http://www.ruby-doc.org/core-2.1.4/String.html](Ruby String). Redtrack will accept and truncate any strings that exceeds the provided length. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Character_types.html](redshift character type documentat))<br/>
```decimal(precision,scale)``` Redtrack expects [http://www.ruby-doc.org/stdlib-1.9.3/libdoc/bigdecimal/rdoc/BigDecimal.html](Ruby BigDecimal). Does not check that the big decimal fits within the columns precision or scale ([http://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html](redshift numeric documentation))
```boolean``` Redtrack expects [http://www.ruby-doc.org/core-2.1.5/FalseClass.html](FalseClass) or [http://www.ruby-doc.org/core-2.1.5/TrueClass.html](TrueClass). Redtrack Does not support "unknown". ([http://docs.aws.amazon.com/redshift/latest/dg/r_Boolean_type.html](redshift boolean documentation))
```date``` Redtrack expects [http://ruby-doc.org/stdlib-2.1.5/libdoc/date/rdoc/Date.html](Ruby Date). ([http://docs.aws.amazon.com/redshift/latest/dg/r_Datetime_types.html](redshift datetime types documentation))
```timestamp``` Redtrack expects [http://www.ruby-doc.org/core-2.1.5/Time.html](Ruby Time). Redshift does not store timezones and Redtrack does not do any timezone conversion, so consider sending data using UTC timezone, otherwise you'll have a possibility of timezone mismatches in the event that servers have inconsistent timezone settings. ([http://docs.aws.amazon.com/redshift/latest/dg/r_Datetime_types.html](redshift datetime types documentation))
